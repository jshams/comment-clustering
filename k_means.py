...
class KMeans():
    ...
    def n_dimension_distance(self, l1, l2):
        total = 0
    ...
    def n_dimension_average(self, x_n_list):
        '''params: x_y_list
        x_y_list: 2d array. list of points ([[x1,y1,z1...], [x2, y2, z2...], [x3, y3, z3...]...])
        returns the average of all points
        '''
        averages = [[] * len(x_n_list)[0]]
        for coordinate in x_n_list:
            for i, x in enumerate(coordinate):
                averages[i].append(x)
        return [sum(average)/len(average) for average in averages]
...
X = []
f = open('sample_blob.txt')
for line in f.readlines():
    split_line = line.split(',')
    x, y = split_line[0], split_line[1]
